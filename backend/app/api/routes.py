...
async def sweet(
    ...
    if existing_sweet:
        raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="Sweet exists")
    else:
        sweet = await sweet_services.create_sweet(
            name=sweet_post.name,
            category=sweet_post.category,
            price=sweet_post.price,
            quantity=sweet_post.quantity
            )
        return {
            "name":sweet.name,
...
async def sweets(
    ...
    return await sweet_services.get_sweets()
...
async def search_sweets(
    ...
    return await sweet_services.search_sweets(
        name=name,
        category=category,
        min_price=min_price
        ,max_price=max_price
...
async def update_sweet(
    ...
    sweet_data: SweetRequest = None,
    current_user=Depends(auth_services.get_current_user)
    ):

    #if the ojectid is valid only then proceed
    if not ObjectId.is_valid(id):
        raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="Invalid sweet ID")
    ...
    #updating sweet
    updated_sweet = await sweet_services.update_sweet(
        id=id,
        update_data={
    ...
    if not updated_sweet:
        raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="Failed to update sweet")

    return SweetResponse(**updated_sweet)#unpacking



@router.delete("/sweets/{id}")
async def delete_sweet(
    id: str = Path(...,description="id of sweet to be deleted"),
     current_user = Depends(auth_services.require_admin)
     ):

    is_deleted = await sweet_services.delete_sweet(id)
    if not is_deleted:
...
async def purchase_sweet(
    ...
    # Check quantity
    if sweet["quantity"] <= 0:
        raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="Sweet is out of stock")
